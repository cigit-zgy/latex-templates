...
curl --fail --location --retry 3 \
  ...
  -o "$work/arial32.exe"
cabextract -q -d "$work/core" "$work/times32.exe"
cabextract -q -d "$work/core" "$work/arial32.exe"
...
rm -f journal/kxtbcas/fonts/*.{ttf,ttc,otf} 2>/dev/null || true
rm -f nsfc-general/fonts/*.{ttf,ttc,otf} 2>/dev/null || true
rm -f nsfc-young/fonts/*.{ttf,ttc,otf} 2>/dev/null || true
...
setup = Path("journal/kxtbcas/scripts/setup-fonts.sh")
setup.write_text(
    """#!/usr/bin/env bash
set -euo pipefail
ROOT=\"$(cd \"$(dirname \"$0\")/..\" && pwd)\"
...
rm -f scripts/vendor-fonts-once.sh .github/workflows/vendor-fonts-once.yml
