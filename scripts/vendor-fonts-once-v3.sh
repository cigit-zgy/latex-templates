...
if r"\def\kxtb@cjkfont@bold{SimSun-Bold.ttf}" not in text:
    raise SystemExit("KXTB SimSun bold token missing")
text = text.replace(
    r"\def\kxtb@cjkfont@bold{SimSun-Bold.ttf}",
    r"\def\kxtb@cjkfont@bold{SimSun.ttf}",
    1,
)
...
set -euo pipefail
ROOT=\"$(cd \"$(dirname \"$0\")/..\" && pwd)\"
for font in TimesNewRoman-Regular.ttf TimesNewRoman-Bold.ttf TimesNewRoman-Italic.ttf TimesNewRoman-BoldItalic.ttf SimSun.ttf; do
...
rm -f scripts/vendor-fonts-once.sh scripts/vendor-fonts-once-v2.sh scripts/vendor-fonts-once-v3.sh .github/workflows/vendor-fonts-once.yml
